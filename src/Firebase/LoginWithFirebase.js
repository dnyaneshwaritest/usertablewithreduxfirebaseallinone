...
const handleSubmit = async (e) => {
  e.preventDefault();
  const result = await firebase.loginWithUser(user.email, user.password)
  console.log(result);
    ...
    <div>
      <form className="registration-form" onSubmit={handleSubmit}>
        ...
        <button className="registration-btn" type="submit" name="submit">
          {/* {user.userId ? "Save Changes" : "Login"} */}
          Login
        </button>
      </form>
      {/* {user.error && <div style={{ color: "red" }}>{user.error}</div>} */}
      <h2>OR</h2>
      <button onClick={firebase.signinWithGoogle} style={{marginLeft:"45%"}}>Signin with Google</button>
    </div>
  );
}
